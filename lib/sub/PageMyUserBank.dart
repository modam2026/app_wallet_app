...
class PageMyUserBank extends StatefulWidget {
  const PageMyUserBank({Key? key}) : super(key: key);

  @override
  State<PageMyUserBank> createState() => _PageMyUserBankState();
}

class _PageMyUserBankState extends State<PageMyUserBank> {
  ...
  Future<List<CachedApplication>> _getInstalledApplications() async {
    ...
    for (var myApp in commonHelper.appDataWithMine) {
      if (myApp["app_user_group"] == "U10" ||
          myApp["app_user_group"] == "U11" ||
          myApp["app_user_group"] == "U12") {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(title: Text("은행 리스트(5/6)")),
